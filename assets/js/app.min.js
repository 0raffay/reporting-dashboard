...
function header() {
  let burgerButton = $('.navbar-toggle')
  ...
  let mobileSubmenuButton = $('.rf--mobileSubmenu-button span, .rf--mobileSubmenu-button > a')
  ...
  mobileSubmenuButton.click(function (e) {
    ...
    $(this).closest(".rf--mobileSubmenu-button").find(mobileSubmenu).slideToggle();
  })
}
header()
...
//   {
//     button: "[data-login]",
//     inputs: "[data-validate-login]",
//     specialFields: {
//       selector: 'validate-login-regex',
//     },
//     errorClass: "error",
...
//     document.querySelector("[data-login-form]").submit();
...
function validateLogin() {
  $("[data-login-form]").on("submit", function (event) {
    ...
    function checkPass() {
      ...
      if ($(".form-control[name='password']").val() == "") {
        $(".form-control[name='password']").addClass("error");
        event.preventDefault();
      } else {
        $(".form-control[name='password']").removeClass("error");
      }
    }
    checkPass()
    checkEmail();

    $(".form-control[name='password'").change(function () {
      checkPass();
    })
...
function validateCreateUser() {
  $("[data-create-user-form]").on("submit", function (event) {
    console.log("working")
    function checkEmail() {
      let emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test($(".create_user_email").val())) {
        $(".create_user_email").addClass("error");
        event.preventDefault();
      } else {
        $(".create_user_email").removeClass("error");
      }
    }

    function checkPass() {
      if ($(".create_user_pass").val() == "") {
        $(".create_user_pass").addClass("error");
        event.preventDefault();
      } else {
        $(".create_user_pass").removeClass("error");
      }
    }
    function checkName() {
      if ($(".create_name").val() == "") {
        $(".create_name").addClass("error");
        event.preventDefault();
      } else {
        $(".create_name").removeClass("error");
      }
    }

    function checkRights() {
      const rightsChecked = $("[name^='client'], [name^='contracts'], [name^='invoice'], [name^='equipment'], [name^='staff'], [name^='report']").is(":checked");
      if (!rightsChecked) {
        $(".rights-error").show();
        event.preventDefault();
      } else {
    ...
    $(".create_user_pass").change(function () {
      checkPass();
    })
    $(".create_name").change(function () {
      checkName();
    })

    $(".create_user_email").change(function () {
      checkEmail();
...
let basic_table = $("#basic_table").DataTable({
  buttons: ["excel", "pdf"]
});

// Add custom classes to adjust column width
$("#basic_table_wrapper .dataTables_length").parent().removeClass("col-md-6").addClass("col-md-4");
$("#basic_table_wrapper #basic_table_filter").parent().removeClass("col-md-6").addClass("col-md-4");
$("#basic_table_wrapper .dt-buttons").addClass("col-md-4");

// Move buttons container to the appropriate column
basic_table.buttons().container().addClass("col-md-2").appendTo("#basic_table_wrapper .row:nth-of-type(1)");
